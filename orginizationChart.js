...
function organizationChart(arr, parent){
    let node = {};

    arr.filter(item => item.parent === parent)
        .forEach(item => node[item.id] = organizationChart(arr, item.id));
    return node;
}

console.log(JSON.stringify(organizationChart(workers, null), null, 2));